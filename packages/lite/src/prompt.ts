...
function buildAgentPrompt(cwd: string, platform: string, web: boolean): string {
	const search = web
...
function buildPlanPrompt(cwd: string, platform: string, web: boolean): string {
	const research = web
...
function buildChatPrompt(cwd: string, platform: string, web: boolean): string {
	if (!web) {
...
export function buildSystemPrompt(options: SystemPromptOptions): string {
	...
	switch (mode) {
		case "plan":
			return buildPlanPrompt(options.cwd, platform, web);
		case "chat":
			return buildChatPrompt(options.cwd, platform, web);
		default:
			return buildAgentPrompt(options.cwd, platform, web);
	}
}
