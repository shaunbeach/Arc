...
export interface LiteModel {
	 ...
	 * arrive from `/props` at connect time, and Arc never spawns a server for it.
	 */
	discover?: boolean;
...
/** Resolution order: `--models`, `$ARC_MODELS`, `./models.yml`, `<app dir>/models.yml`. */
export function findModelsFile(search: ModelsFileSearch = {}): string {
	...
	const pinned = search.explicit ?? env.ARC_MODELS ?? env.PI_MODELS;
	if (pinned) {
	...
	if (!found) {
		...
		throw new ModelsConfigError(
			`No models.yml found. Looked in:\n${searched}\nRun arc --init to create a starter file, or pass --models <path> or set ARC_MODELS.`,
		);
	}
	return found;
}
...
export function parseModelsConfig(text: string, path: string, options: ParseModelsOptions = {}): ModelsConfig {
	...
	for (const [providerName, provider] of Object.entries(root.providers)) {
		...
		if (api !== undefined && api !== "openai-completions") {
			warnings.push(`${at}.api is "${api}"; Arc always uses /v1/chat/completions.`);
		}
