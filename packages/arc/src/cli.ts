...
import { getAppDir, migrateLegacyAppDir } from "./config/paths.ts";
...
const HELP = `Usage:
  arc [options]                Start an interactive session
  arc [options] -p <prompt>    Run one prompt with tools and print the reply
...
Options:
  ...
  -r, --resume                Pick a saved session to resume (interactive)
      ...
      --models <path>         models.yml location (default: ./models.yml, then ~/.arc/models.yml)
      --init                  Write a starter ~/.arc/models.yml and exit
...
Interactive commands: /agent  /plan  /chat  /web [on|off]  /rag [on|off]  /ponytail [off|lite|full|ultra]  /model [name]  /mode [thinking|instruct]  /serve [name]  /disconnect  /compact  /clear  /resume [number|name|id]  /name <text>  /supervise [plan.md|resume|stop]  /audit [critic]  /usage  /quit
`;
...
function initModels(appDir: string): number {
	...
	process.stdout.write(`Wrote ${path}\nSet the GGUF path and llama-server arguments in it, then run Arc.\n`);
	return 0;
}
...
async function main(argv: string[]): Promise<number> {
	...
	const moved = migrateLegacyAppDir();
